<script lang="ts">
  ...
  function formatTimeAmPm(decimalHour: number): string {
  ...
  function formatStartTime(startHour: number): string {
    return formatTimeAmPm(startHour);
  }

  function formatEndTime(startHour: number, duration: number): string {
    return formatTimeAmPm(startHour + duration);
  }
...
<div class="calendar-container">
      ...
      {#each Array(7) as _, dayIdx (dayIdx)}
        <div class="all-day-cell">
          {#each allDayByDay[dayIdx] as event (event.id || event.title)}
            ...
            <button
              class="all-day-event event-enter"
  ...
  <div class="time-grid-scroll" bind:this={scrollContainer}>
    ...
    >
      ...
      <!-- Day Columns (each with its own hour lines) -->
      {#each Array(7) as _, dayIdx (dayIdx)}
        {@const todayIdx = getTodayDayIndex()}
        <div class="day-col" class:day-col--today={dayIdx === todayIdx}>
          <!-- Hour lines inside each column — always behind events -->
          {#each hours as _, hourIdx (hourIdx)}
            <div class="hour-line" style="top: {hourIdx * HOUR_HEIGHT}px;" aria-hidden="true"></div>
          {/each}

          {#if dayIdx === todayIdx}
            <TimeIndicator startHour={START_HOUR} hourHeight={HOUR_HEIGHT} />
          {/if}
          ...
          {#each layoutByDay[dayIdx] as event (event.id || event.title)}
            ...
            {@const displayStartHour = isResizing ? resizeState!.currentStartHour : event.startHour}
...
<style lang="scss">
  ...
  .event-enter {
    animation: eventFadeIn 200ms ease both;
  ...
  .day-header {
    ...
    &--today {
      background: rgba(61, 143, 164, 0.04);
    }
  ...
  .cal-event {
    ...
    box-shadow: inset 0 0 0 1000px var(--ev-bg);
