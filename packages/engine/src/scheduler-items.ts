...
export function habitsToScheduleItems(
  ...
  for (const habit of habits) {
    ...
    if (habit.frequency === Frequency.Daily || (habit.frequency as string) === 'custom') {
      ...
      const applicableDays = habit.frequencyConfig?.days ?? [...DEFAULT_WEEKLY_DAY];

      // Use ISO-year-qualified week keys to handle year boundaries correctly.
      ...
      for (const day of days) {
        ...
        const dayName = getDayAbbrev(day, tz);
        const dayWeekKey = `${weekKey}-${dayName}`;
        if (scheduledWeeks.has(dayWeekKey)) continue;
        ...
        items.push({
          ...
          skipBuffer: habit.skipBuffer ?? false,
          forced: habit.forced,
          dependsOn: habit.dependsOn ? `${habit.dependsOn}__${dayStr}` : null,
        });
      }
    } else if (habit.frequency === Frequency.Monthly) {
      for (const day of days) {
        if (!isMonthlyTargetDay(day, tz, habit.frequencyConfig)) continue;
